...
```text
├── app/                    # Flask Backend (The "Brain")
│   ├── api/                # Route definitions (Transactions, Admin, Variables)
│   ├── services/           # Business Logic: KPI math, Commission rules, Excel parsing
│   ├── utils/              # Math helpers and general utilities
│   └── models.py           # Database Schema
├── src/                    # React Frontend (The "Dashboard")
│   ├── features/           # Domain modules (Auth, Admin, Transactions)
│   ├── components/         # UI library (Shadcn/UI components)
│   └── lib/                # API clients and formatters
├── migrations/             # Database version history (Alembic)
├── tools/                  # DevOps & infrastructure utilities
│   ├── scripts/            # Deployment, backup, and health check scripts
│   └── supabase_rls_scripts/ # SQL scripts for database-level security (RLS)
└── api/index.py            # Entry point for Vercel production
```
---
...
## 🛡️ Standardization & Development Rules
...
3. **Security Protocol:** While tables are currently tagged as "unrestricted" in Supabase, any security updates should be implemented via the scripts in `tools/supabase_rls_scripts/`. This centralizes the Row Level Security (RLS) logic.
...
This project uses **automated CI/CD pipelines** for safe database migrations and deployments.
...
- **Setup**: See [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) for GitHub Secrets configuration
- **Migrations**: See [docs/DATABASE_MIGRATIONS.md](docs/DATABASE_MIGRATIONS.md) for best practices
- **Rollback**: See [docs/ROLLBACK.md](docs/ROLLBACK.md) for emergency procedures

### Important Rules
- ⚠️ **Never run migrations in serverless functions** (causes race conditions)
- ⚠️ **Always test in staging first** (push to `develop` before `main`)
- ⚠️ **Migrations use Port 5432** (direct connection), runtime uses Port 6543 (pooler)
